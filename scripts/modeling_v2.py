...
def init_association_matrix(belief_network, propositions = ['A','B','C','D']):
    ...
    propositions_w_sign = ["+"+p for p in propositions] + ["0"+p for p in propositions] + ["-"+p for p in propositions] # add the signs
    ...
    for e in belief_network.edges(): # fill up 
        for s in [*itertools.product(['+','0','-'],['+','0','-'])]:
...
def bayesian(e1, e2, association_matrix):
    ...
    evidence = 0
    for s in signs:
        evidence += association_matrix[f'{s}{e2[1]}'].sum()
    ...
    # no association
    joint_3 = association_matrix[f'0{e2[1]}'][f'+{e1[1]}'] + association_matrix[f'0{e2[1]}'][f'-{e1[1]}'] + association_matrix[f'-{e2[1]}'][f'0{e1[1]}'] + association_matrix[f'+{e2[1]}'][f'0{e1[1]}'] + association_matrix[f'0{e2[1]}'][f'0{e1[1]}'] + association_matrix[f'0{e2[1]}'][f'0{e1[1]}']
    
    likelihood_pos_rel = joint_1/evidence
    likelihood_neg_rel = joint_2/evidence
    likelihood_no_rel = joint_3/evidence
    
    return likelihood_pos_rel, likelihood_neg_rel, likelihood_no_rel
...
def infer_relationship(message_to_convey, association_matrix):
    ...
    likelihood_pos_rel, likelihood_neg_rel, likelihood_no_rel = bayesian(e1, e2, association_matrix)

    arr = np.array([likelihood_pos_rel, likelihood_neg_rel, likelihood_no_rel])
    
    # if two of them have the same likelihood, choose one randomly
    highest = np.random.choice(np.where(arr==max(arr))[0],1)[0]
    
    # first one is positive, second one is negative, third one is no relationship
    return {0:1, 1:-1, 2:0}[highest]
...
def deduce_message_to_convey(beliefs_to_convey, agent_beliefs, gamma):
    
    message_to_convey = {}

    for i, b in enumerate(beliefs_to_convey):
        if (agent_beliefs[beliefs_to_convey[i]] < gamma) & (agent_beliefs[beliefs_to_convey[i]] > -1*gamma):
            message_to_convey[f"e{i+1}"] = f"0{beliefs_to_convey[i]}"
        elif agent_beliefs[beliefs_to_convey[i]] < -1*gamma:
...
def interpretative_agreement(propositions, agents):
    ...
    for matrix in [v['association_matrix'] for v in agents.values()]:
        ...
        for p1, p2 in proposition_combinations:
            likelihood_pos_rel, likelihood_neg_rel, likelihood_no_rel = bayesian(f'+{p1}', f'+{p2}', matrix)
            posteriors.append(likelihood_pos_rel)
            posteriors.append(likelihood_neg_rel)
            posteriors.append(likelihood_no_rel)
...
def simulate_multiple_agents(sim_no, random_seed, n_beliefs, m, gamma, n_nodes_to_connect, n_agents = 20, last_step = 500000, is_composed = False):
    ...
    for t in tqdm(simulation_steps):
        ...
        message_to_convey = deduce_message_to_convey(beliefs_to_convey, agents[i]['beliefs'], gamma) # deduce the message to convey
        ...
        if t % 1000 == 0:
            ...
            track[t]['belief_networks'] = beliefs
            track[t]['association_matrix'] = {k:v['association_matrix'] for k,v in agents.items()}
